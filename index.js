...
async function processBuyBuySell(buyBuySell){
    for(let i =0; i < buyBuySell.length; i++){
        ...
        if(crossRate > PROFITABILITY){
            ...
            const coins = account.balances.filter(b => (b.asset).free !== -1)

            if(AMOUNT <= parseFloat(coins.find(c => c.asset === QUOTE).free)){

                const buyOrder = await api.newOrderBuy(candidate.buy1.symbol, (AMOUNT/priceBuy1).toFixed(2))
               ...
               if(buyOrder.status === 'FILLED'){
                    const buyOrder2 = await api.newOrderBuy(candidate.buy2.symbol, (buyOrder.origQty/priceBuy2).toFixed(2))
    ...
    for(let i=0; i < buySellsell.length; i++){
        ...
        if(crossRate > PROFITABILITY){
            ...
            if(AMOUNT <= parseFloat(coins.find(c => c.asset === QUOTE).free)){
                ...
                if(buyOrder.status === 'FILLED'){
                   const sellOrder = await api.newOrderSell(candidate.sell1.symbol, (buyOrder.origQty*priceSell1).toFixed(2))
                    console.log(`Order: ${sellOrder.orderId}`)
                    console.log(`Satus: ${sellOrder.status}`)
